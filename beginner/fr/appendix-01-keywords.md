...
Aucun de ces mots ne peut être détourné, si bien qu'il conviendrait à votre code. Essayez de nommer une variable `$class` : celle-là passe, en fait, car les mots réservés ne bloquent que les identifiants *nus*, pas les noms de variables derrière le `$`. Essayez de nommer une fonction `list()` ou une classe `Match`, et PHP vous arrêtera à l'analyse du fichier, pas à l'exécution. Mieux vaut là qu'en production.
