...
Type the code along as you go rather than copying it in one block at the end. The value of this chapter is in watching the program change shape across sections (clumsy first, then modular, then tested, then polished) not in the final file on its own. And keep the project around once you're done: [Chapter 15](ch15-00-functional-features.md) comes back to phpgrep and gives it one more upgrade, once you've met a PHP feature it's specifically well suited to.
